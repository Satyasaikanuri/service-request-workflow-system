...
const AdminDashboard = {
    ...
    updateAssignmentMonitor: function (requests, users) {
        ...
        tbody.innerHTML = recent.map(r => `
            <tr>
                ...
                <td style="color: #94a3b8;">${escapeHTML(r.requestType ? r.requestType.name : 'General')}</td>
                <td style="color: #fbbf24; font-weight:500;">${escapeHTML(r.assignedAgent.username)}</td>
    ...
    updateApproverPerformance: function (users, requests) {
        ...
        tbody.innerHTML = stats.map(s => `
            <tr>
                <td style="font-weight: 500; color: #fff;">${escapeHTML(s.name)}</td>
                <td style="color: #94a3b8;">${escapeHTML(s.dept)}</td>
    ...
    updateAuditLogs: function (audits) {
        ...
        consoleDiv.innerHTML = recent.map(log => {
            ...
            <div class="audit-entry">
                ...
                <span class="audit-action" style="color: ${actionColor}">${escapeHTML(log.action)}</span>
                <span style="color: #cbd5e1;">Req #${log.requestId}: ${escapeHTML(log.remarks)}</span>
                <span style="color: #64748b; font-size: 0.75rem;">(${log.performedBy ? escapeHTML(log.performedBy.username) : 'SYSTEM'})</span>
            </div>
            `;
        }).join('');
    },
            ...
            <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #334155; display: flex; gap: 0.75rem; align-items: start;">
                ...
                <div>
                    <div style="font-size: 0.85rem; font-weight: 500; color: #f1f5f9;">
                         Req #${log.requestId}: ${escapeHTML(log.action)}
                    </div>
                    <div style="font-size: 0.75rem; color: #94a3b8;">
                        ${escapeHTML(log.remarks) || 'Update'} - ${new Date(log.timestamp).toLocaleTimeString()}
                    </div>
                </div>
            </div>
        `).join('');
        ...
        try {
            ...
            tbody.innerHTML = departments.map(d => `
                <tr style="border-bottom: 1px solid #1e293b;">
                    ...
                    <td style="padding: 1rem; color: #f8fafc; font-weight: 500;">${escapeHTML(d.name)}</td>
                    <td style="padding: 1rem; color: #94a3b8;">${escapeHTML(d.description) || '-'}</td>
        ...
        try {
            ...
            tbody.innerHTML = users.map(u => `
                <tr style="border-bottom: 1px solid #1e293b;">
                    ...
                    <td style="padding: 1rem; color: #f8fafc; font-weight: 500;">${escapeHTML(u.username)}</td>
                    <td style="padding: 1rem; color: #94a3b8;">${escapeHTML(u.email)}</td>
                    <td style="padding: 1rem;"><span class="badge" style="background: rgba(59, 130, 246, 0.1); color: #3b82f6;">${escapeHTML(u.role.name)}</span></td>
                    <td style="padding: 1rem; color: #94a3b8;">${u.department ? escapeHTML(u.department.name) : '-'}</td>
                    <td style="padding: 1rem; text-align: right;">
        ...
        try {
            ...
            tbody.innerHTML = approvers.map(u => `
                <tr style="border-bottom: 1px solid #1e293b;">
                    ...
                    <td style="padding: 1rem; color: #f8fafc; font-weight: 500;">${escapeHTML(u.username)}</td>
                    <td style="padding: 1rem; color: #94a3b8;">${escapeHTML(u.email)}</td>
                    <td style="padding: 1rem; color: #94a3b8;">${u.department ? escapeHTML(u.department.name) : '<span class="text-warning">Unassigned</span>'}</td>
                    <td style="padding: 1rem; text-align: right;">
                        <button class="btn btn-sm" onclick='AdminDashboard.openUserModal(${JSON.stringify(u).replace(/'/g, "&#39;")})'
                            style="background: transparent; color: #3b82f6; margin-right: 0.5rem;" title="Edit">
                            <i class="fas fa-edit"></i>
