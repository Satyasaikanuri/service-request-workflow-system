    ...
    loadData: async function (silent = false) {
        ...
        try {
            ...
            const [requests, notifs] = await Promise.all([
                API.get('/requests/my'),
                API.get('/requests/notifications') // Using the new endpoint or just logic? We created /notifications
            ]);
    ...
    switchView: function (viewName) {
        ...
        if (viewName === 'reports') {
            ...
            if (viewName === 'dashboard') {
                ...
                this.renderCharts(); // Re-render dashboard specific charts
            } else {
                ...
                // Set Title
                if (viewName === 'myrequests') tableTitle.innerText = "All Requests";
                else if (viewName === 'pending') tableTitle.innerText = "Pending Requests";
                else if (viewName === 'approved') tableTitle.innerText = "Approved Requests";
                else if (viewName === 'rejected') tableTitle.innerText = "Rejected Requests";
            }
        }
    ...
    renderTable: function () {
        ...
        tbody.innerHTML = sorted.map(r => {
            ...
            <tr onclick="UserDashboard.openTimeline(${r.id})" style="cursor: pointer;" class="hover:bg-gray-50">
                ...
                <td style="padding: 1rem; font-weight: 600;">${r.title}</td>
                <td style="padding: 1rem; color: #64748b;">${r.department.name}</td>
            ...
            <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #f1f5f9; display: flex; gap: 0.75rem; align-items: start;">
                 ...
                 <div>
                    <div style="font-size: 0.85rem; font-weight: 500; color: #334155;">
                        Req #${n.requestId}: ${actionText}
                    </div>
                    <div style="font-size: 0.75rem; color: #64748b;">
                        ${n.remarks || 'Status updated'} - ${this.timeAgo(new Date(n.timestamp))}
                    </div>
                 </div>
            </div>
        `}).join('');
        ...
        try {
            ...
            container.innerHTML = audits.map(a => `
                <div class="timeline-item active">
                    ...
                    <div style="color: #64748b; font-size: 0.9rem;">By: ${a.performedBy ? a.performedBy.username : 'SYSTEM'}</div>
                    <div style="color: #64748b; font-size: 0.9rem;">${a.remarks || ''}</div>
