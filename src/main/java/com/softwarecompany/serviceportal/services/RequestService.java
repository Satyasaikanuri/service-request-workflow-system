...
public class RequestService {
    ...
    public List<ServiceRequest> getMyRequests() {
        UserDetailsImpl userDetails = getCurrentUser();
        return requestRepository.findByUserIdAndDeletedAtIsNull(userDetails.getId());
    }
    ...
    public List<ServiceRequest> getRequestsForApprover() {
        ...
        return requestRepository.findByDepartmentIdAndDeletedAtIsNull(userDetails.getDepartmentId());
    }
    ...
    public List<ServiceRequest> getRequestsAssignedToMe() {
        UserDetailsImpl userDetails = getCurrentUser();
        return requestRepository.findByAssignedAgentIdAndDeletedAtIsNull(userDetails.getId());
    }
    ...
    public void deleteRequest(Long requestId) {
        ...
        // Audit
        auditService.logAction(request.getId(), "DELETED", request.getStatus().name(), "DELETED",
                userRepository.findById(userDetails.getId()).orElse(null),
                "Request soft-deleted by " + (isAdmin ? "Admin" : "User"));
    }
    ...
    public org.springframework.data.domain.Page<ServiceRequest> getMyRequestsPaged(
            int page, int size, String search, String status, String priority, String sortBy, String direction) {
        UserDetailsImpl userDetails = getCurrentUser();
        return getRequestsPaged(userDetails.getId(), null, null, page, size, search, status, priority, sortBy, direction);
    }

    public org.springframework.data.domain.Page<ServiceRequest> getRequestsForApproverPaged(
            int page, int size, String search, String status, String priority, String sortBy, String direction) {
        UserDetailsImpl userDetails = getCurrentUser();
        if (userDetails.getDepartmentId() == null) {
            return org.springframework.data.domain.Page.empty();
        }
        return getRequestsPaged(null, userDetails.getDepartmentId(), null, page, size, search, status, priority, sortBy, direction);
    }

    public org.springframework.data.domain.Page<ServiceRequest> getAllRequestsForAdminPaged(
            int page, int size, String search, String status, String priority, String sortBy, String direction) {
        return getRequestsPaged(null, null, null, page, size, search, status, priority, sortBy, direction);
    }

    public org.springframework.data.domain.Page<ServiceRequest> getRequestsAssignedToMePaged(
            int page, int size, String search, String status, String priority, String sortBy, String direction) {
        UserDetailsImpl userDetails = getCurrentUser();
        return getRequestsPaged(null, null, userDetails.getId(), page, size, search, status, priority, sortBy, direction);
    }

    public org.springframework.data.domain.Page<ServiceRequest> getRequestsPaged(
            Long userId, Long departmentId, Long assignedAgentId,
            int page, int size, String search, String status, String priority,
        ...
        org.springframework.data.domain.Pageable pageable = org.springframework.data.domain.PageRequest.of(page, pageSize, sort);
        
        org.springframework.data.jpa.domain.Specification<ServiceRequest> spec = (root, query, cb) -> {
            ...
            if (status != null && !status.trim().isEmpty()) {
                try {
                    ServiceRequest.RequestStatus statusEnum = ServiceRequest.RequestStatus.valueOf(status.toUpperCase());
            ...
            if (priority != null && !priority.trim().isEmpty()) {
                try {
                    ServiceRequest.Priority priorityEnum = ServiceRequest.Priority.valueOf(priority.toUpperCase());
    ...
    public void exportRequestsCsv(java.io.PrintWriter writer, Long userId, Long departmentId) {
        org.springframework.data.jpa.domain.Specification<ServiceRequest> spec = (root, query, cb) -> {
            java.util.List<jakarta.persistence.criteria.Predicate> predicates = new java.util.ArrayList<>();
            predicates.add(cb.isNull(root.get("deletedAt")));
        ...
        List<ServiceRequest> requests = requestRepository.findAll(spec, org.springframework.data.domain.Sort.by(org.springframework.data.domain.Sort.Direction.DESC, "createdAt"));
        
        writer.println("ID,Title,Description,Department,Priority,Status,SLA Hours,Due Date,Assigned Agent,Created At");
        
        for (ServiceRequest r : requests) {
            String agent = r.getAssignedAgent() != null ? r.getAssignedAgent().getUsername() : "Unassigned";
            writer.println(String.format("%d,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%d,\"%s\",\"%s\",\"%s\"",
                    r.getId(),
